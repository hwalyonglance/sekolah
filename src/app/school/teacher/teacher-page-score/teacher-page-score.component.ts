...
export class TeacherPageScoreComponent implements AfterViewInit, OnInit {
	...
	get createUrl() {
		...
		slash = '/'
		const createUrl = ['/'+teacher, penugasan, teacherAssignment_id, nilai, 'buat']
	...
	get editUrl() {
		...
		slash = '/'
		const createUrl = [slash, teacher, penugasan, guru, teacherAssignment_id, nilai, 'buat']
		// console.log(this._router.url.split('/'), createUrl)
		return createUrl
	}
	get showRowMenuTrigger() {
		return this.teacherAssignmentV.teacherAssignmentStatus === 'Aktif'
	}
